...
export const Kravspecifikation = () => {
  ...
  const [ickeFunktionella, setIckeFunktionella] = useState<
    IckeFunktionelltKrav[]
  >([]);
  ...
  const handleSave = async () => {
    try {
      await api.saveKravspec({
        funktionella,
        ickefunktionella: ickeFunktionella,
      });
      toast({
        title: "Sparat!",
        description: "Kravspecifikation har sparats.",
      });
    } catch (error) {
  ...
  const updateFunktionellt = (
    id: string,
    field: keyof FunktionelltKrav,
    value: string
  ) => {
    setFunktionella(
      funktionella.map((k) => (k.id === id ? { ...k, [field]: value } : k))
    );
  };
  ...
  const updateIckeFunktionellt = (
    id: string,
    field: keyof IckeFunktionelltKrav,
    value: string
  ) => {
    setIckeFunktionella(
      ickeFunktionella.map((k) => (k.id === id ? { ...k, [field]: value } : k))
    );
  };
                  ...
                  <tr key={krav.id} className="border-b">
                    ...
                    <td className="p-2">
                      <Input
                        value={krav.krav}
                        onChange={(e) =>
                          updateFunktionellt(krav.id, "krav", e.target.value)
                        }
                    ...
                    <td className="p-2">
                      <Select
                        value={krav.moscow}
                        onValueChange={(value) =>
                          updateFunktionellt(krav.id, "moscow", value)
                        }
                      >
                        <SelectTrigger>
                          <SelectValue />
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b">
                  ...
                  <th className="text-left p-2 font-semibold">Beskrivning</th>
                  <th className="text-left p-2 font-semibold w-40">
                    Prioritet
                  </th>
                  <th className="w-10"></th>
                </tr>
                  ...
                  <tr key={krav.id} className="border-b">
                    ...
                    <td className="p-2">
                      <Select
                        value={krav.kategori}
                        onValueChange={(value) =>
                          updateIckeFunktionellt(krav.id, "kategori", value)
                        }
                      >
                        <SelectTrigger>
                          <SelectValue />
                    ...
                    <td className="p-2">
                      <Select
                        ...
                        onValueChange={(value) =>
                          updateIckeFunktionellt(krav.id, "prioritet", value)
                        }
                      >
                        <SelectTrigger>
                          <SelectValue />
